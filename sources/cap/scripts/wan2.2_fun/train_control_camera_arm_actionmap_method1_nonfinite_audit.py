...
def main():
    ...
    if args.method1_sample_loss_dir:
        ...
        if args.resume_from_checkpoint and first_epoch > 0:
    ...
    # This audit is intentionally opt-in and exists only in the diagnostic script.
    # It stops before a non-finite update can be applied, while retaining the last
    # four accumulated microbatches that contributed to the update.
    cap_nonfinite_audit_enabled = os.environ.get("CAP_NONFINITE_AUDIT", "0") == "1"
    cap_nonfinite_audit_start_step = int(
        os.environ.get("CAP_NONFINITE_AUDIT_START_STEP", "0")
    )
    cap_nonfinite_audit_records = []

    if cap_nonfinite_audit_enabled and accelerator.is_main_process:
        print(
            "CAP nonfinite audit enabled: "
            f"start_global_step={cap_nonfinite_audit_start_step}",
            flush=True,
        )

    def cap_nonfinite_audit(stage, *, inspect_gradients):
        if (
            not cap_nonfinite_audit_enabled
            or global_step < cap_nonfinite_audit_start_step
        ):
            return

        named_tensors = []
        for name, parameter in transformer3d.named_parameters():
            if not parameter.requires_grad:
                continue
            value = parameter.grad if inspect_gradients else parameter
            if value is not None:
                named_tensors.append((name, value.detach()))

        if named_tensors:
            finite_flags = torch.stack(
                [torch.isfinite(value).all() for _, value in named_tensors]
            )
            local_finite = finite_flags.all()
        else:
            finite_flags = torch.empty(
                0, device=accelerator.device, dtype=torch.bool
            )
            local_finite = torch.ones(
                (), device=accelerator.device, dtype=torch.bool
            )

        all_rank_finite = accelerator.gather(
            local_finite.to(dtype=torch.int64).reshape(1)
        )
        if bool(all_rank_finite.all().item()):
            return

        bad_indices = (~finite_flags).nonzero(as_tuple=False).flatten().tolist()
        tensor_kind = "gradient" if inspect_gradients else "parameter"
        print(
            "CAP nonfinite audit failure: "
            f"rank={accelerator.process_index} stage={stage} "
            f"global_step={global_step} epoch={epoch} dataloader_step={step} "
            f"tensor_kind={tensor_kind} all_rank_finite="
            f"{all_rank_finite.tolist()} local_bad_tensor_count={len(bad_indices)}",
            flush=True,
        )
        for tensor_index in bad_indices[:16]:
            name, value = named_tensors[tensor_index]
            ...
            finite_abs_max = float(
                torch.nan_to_num(
                    value.float(), nan=0.0, posinf=0.0, neginf=0.0
                ).abs().max().item()
            )
            print(
                "CAP nonfinite tensor: "
                f"rank={accelerator.process_index} stage={stage} name={name} "
                f"shape={tuple(value.shape)} dtype={value.dtype} "
                f"nan={nan_count} posinf={posinf_count} neginf={neginf_count} "
                f"finite_abs_max={finite_abs_max:.9g}",
                flush=True,
            )
        print(
            "CAP nonfinite contributing_microbatches: "
            f"rank={accelerator.process_index} records="
            f"{json.dumps(cap_nonfinite_audit_records, ensure_ascii=True)}",
            flush=True,
        )
        accelerator.wait_for_everyone()
        raise FloatingPointError(
            "CAP nonfinite audit stopped before an invalid optimizer update; "
            f"stage={stage} global_step={global_step}."
        )

    def cap_nonfinite_audit_clip_norm(norm_sum):
        if (
            not cap_nonfinite_audit_enabled
            or global_step < cap_nonfinite_audit_start_step
        ):
        ...
        if not torch.is_tensor(norm_sum):
            norm_sum = torch.tensor(norm_sum, device=accelerator.device)
        all_rank_finite = accelerator.gather(
            torch.isfinite(norm_sum).all().to(dtype=torch.int64).reshape(1)
        )
        if bool(all_rank_finite.all().item()):
            return
        print(
            "CAP nonfinite audit failure: "
            f"rank={accelerator.process_index} stage=clip_norm "
            f"global_step={global_step} epoch={epoch} dataloader_step={step} "
            f"local_norm={norm_sum.detach().float().item()} "
            f"all_rank_finite={all_rank_finite.tolist()}",
            flush=True,
        )
        accelerator.wait_for_everyone()
        raise FloatingPointError(
            "CAP nonfinite audit stopped after clip_grad_norm_; "
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(transformer3d):
                ...
                if (
                    cap_nonfinite_audit_enabled
                    and global_step >= cap_nonfinite_audit_start_step
                ):
                    metadata_indices = batch.get("idx")
                    cap_nonfinite_audit_records.append(
                        {
                            "epoch": int(epoch),
                            "dataloader_step": int(step),
                            "global_step": int(global_step),
                            "metadata_indices": (
                                metadata_indices.detach().cpu().reshape(-1).tolist()
                                if metadata_indices is not None
                                else []
                            ),
                            "loss": float(loss.detach().float().item()),
                            "active_ratio": float(
                                method1_active_mask.detach().float().mean().item()
                            )
                            if method1_active_mask is not None
                            else 0.0,
                            "rho_max": float(
                                method1_rho.detach().float().max().item()
                            )
                            if method1_rho is not None
                            else 1.0,
                        }
                    )
                ...
                if accelerator.sync_gradients:
                    ...
                    cap_nonfinite_audit(
                        "pre_clip_gradient", inspect_gradients=True
                    )
                    ...
                    cap_nonfinite_audit_clip_norm(norm_sum)
                    cap_nonfinite_audit(
                        "post_clip_gradient", inspect_gradients=True
                    )
                    if not args.use_deepspeed and not args.use_fsdp and args.report_model_info and accelerator.is_main_process:
                ...
                if accelerator.sync_gradients:
                    cap_nonfinite_audit(
                        "post_optimizer_parameter", inspect_gradients=False
                    )
                lr_scheduler.step()
                optimizer.zero_grad()
                if accelerator.sync_gradients:
                    cap_nonfinite_audit_records.clear()
