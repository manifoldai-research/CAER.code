...
Optional environment:
  ...
  CAP_STAGE_LOCAL_SITE      Optional local site-packages staging (default: 0).
...
# This is deliberately scoped to the one previously unstable Camera ablation.
# Keep every other formal ablation on its historical, unguarded update path.
CAP_REQUESTED_ACTION_DROPOUT="0.10"
CAP_REQUESTED_TAU_S="0.50"
for ((cap_arg_index = 0; cap_arg_index < ${#CAP_EXTRA_TRAIN_ARGS[@]}; cap_arg_index++)); do
    cap_arg="${CAP_EXTRA_TRAIN_ARGS[$cap_arg_index]}"
    case "$cap_arg" in
        --method1_action_dropout_prob)
            if (( cap_arg_index + 1 < ${#CAP_EXTRA_TRAIN_ARGS[@]} )); then
                cap_arg_index=$((cap_arg_index + 1))
                CAP_REQUESTED_ACTION_DROPOUT="${CAP_EXTRA_TRAIN_ARGS[$cap_arg_index]}"
            fi
            ;;
        --method1_action_dropout_prob=*)
            CAP_REQUESTED_ACTION_DROPOUT="${cap_arg#*=}"
            ;;
        --method1_tau_s)
            if (( cap_arg_index + 1 < ${#CAP_EXTRA_TRAIN_ARGS[@]} )); then
                cap_arg_index=$((cap_arg_index + 1))
                CAP_REQUESTED_TAU_S="${CAP_EXTRA_TRAIN_ARGS[$cap_arg_index]}"
            fi
            ;;
        --method1_tau_s=*)
            CAP_REQUESTED_TAU_S="${cap_arg#*=}"
            ;;
    esac
done
...
RUN_CAP="$SCRIPT_DIR/run_cap_train.sh"
