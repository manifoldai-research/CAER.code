...
def method1_focused_flow_loss(
    ...
    """Compute the per-sample Method1 flow-matching loss in float32.

    ``effect_map`` is S = ||P_on - P_null||_2 over prediction channels.
    ``active_mask`` selects samples whose action condition was retained; all
    inactive samples receive uniform weight rho=1.
    """
