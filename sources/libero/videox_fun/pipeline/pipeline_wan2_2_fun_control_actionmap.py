...
class Wan2_2FunControlPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        control_camera_mask: Optional[torch.FloatTensor] = None,
        action_map_mask: Optional[torch.FloatTensor] = None,
        action_map_depth_video: Optional[torch.FloatTensor] = None,
        action_map_depth_scale: float = 1.0,
        arm_action: Optional[torch.FloatTensor] = None,
        arm_action_mask: Optional[torch.FloatTensor] = None,
        ...
        use_empty_control_latents: bool = True,
        shift: int = 5,
        ...
        if control_camera_video is not None:
            ...
            action_map_latents = self.prepare_control_latents(
                None,
                control_video,
                batch_size,
                height,
            ...
            if action_map_depth_video is not None:
                depth_video_length = action_map_depth_video.shape[2]
                action_map_depth_video = self.image_processor.preprocess(
                    rearrange(action_map_depth_video, "b c f h w -> (b f) c h w"),
                    height=height,
                    width=width,
                )
                action_map_depth_video = action_map_depth_video.to(dtype=torch.float32)
                action_map_depth_video = rearrange(
                    action_map_depth_video,
                    "(b f) c h w -> b c f h w",
                    f=depth_video_length,
                )
                action_map_depth_latents = self.prepare_control_latents(
                    None,
                    action_map_depth_video,
                    batch_size,
                    height,
                ...
                depth_frames_to_use = max(action_map_latents.size(2) - 1, 0)
                if depth_frames_to_use > 0 and action_map_depth_latents.size(2) > 0:
                    action_map_depth_latents = action_map_depth_latents.to(
                        device=action_map_latents.device,
                        dtype=action_map_latents.dtype,
                    )
                    if action_map_depth_latents.size(2) == 1 and depth_frames_to_use > 1:
                        action_map_depth_latents = action_map_depth_latents.expand(
                            -1, -1, depth_frames_to_use, -1, -1
                        )
                    depth_frames_to_use = min(depth_frames_to_use, action_map_depth_latents.size(2))
                    action_map_latents[:, :, 1 : 1 + depth_frames_to_use] = (
                        action_map_latents[:, :, 1 : 1 + depth_frames_to_use]
                        + action_map_depth_scale * action_map_depth_latents[:, :, :depth_frames_to_use]
                    )
            if action_map_mask is None:
                action_map_mask = torch.ones(batch_size, device=device, dtype=torch.float32)
            if init_video is not None:
                action_gate = action_map_mask.to(device=masked_video_latents.device, dtype=masked_video_latents.dtype).view(-1, 1, 1, 1, 1)
                base_masked_video_latents = masked_video_latents.clone()
                action_masked_video_latents = masked_video_latents.clone()
                frames_to_use = min(action_masked_video_latents.size(2), action_map_latents.size(2))
                if frames_to_use > 1:
                    action_masked_video_latents[:, :, 1:frames_to_use] = action_map_latents[:, :, 1:frames_to_use]
                masked_video_latents = action_gate * action_masked_video_latents + (1.0 - action_gate) * base_masked_video_latents
            control_video_latents = None
            control_camera_latents = None
        else:
            control_video_latents = torch.zeros_like(latents).to(device, weight_dtype) if use_empty_control_latents else None
            control_camera_latents = None
            ...
            for i, t in enumerate(timesteps):
                ...
                if control_camera_video is not None:
                    ...
                    if control_camera_mask is not None:
                        control_camera_mask_input = (
                            torch.cat([control_camera_mask] * 2) if do_classifier_free_guidance else control_camera_mask
                        ).to(device=device, dtype=torch.float32)
                    else:
                        control_camera_mask_input = None
                else:
                ...
                if arm_action is not None:
                    arm_action_input = (
                        torch.cat([arm_action] * 2) if do_classifier_free_guidance else arm_action
                    ).to(device, weight_dtype)
                else:
                    arm_action_input = None

                if arm_action_mask is not None:
                    arm_action_mask_input = (
                        torch.cat([arm_action_mask] * 2) if do_classifier_free_guidance else arm_action_mask
                    ).to(device)
                else:
                    arm_action_mask_input = None

                if action_map_mask is not None:
                    action_map_mask_input = (
                        torch.cat([action_map_mask] * 2) if do_classifier_free_guidance else action_map_mask
                    ).to(device=device, dtype=torch.float32)
                else:
                    action_map_mask_input = torch.zeros(latent_model_input.size(0), device=device, dtype=torch.float32)
                ...
                local_transformer._current_action_map_mask = action_map_mask_input
                with torch.cuda.amp.autocast(dtype=weight_dtype), torch.cuda.device(device=device):
                    noise_pred = local_transformer(
                        ...
                        y_camera_mask=control_camera_mask_input,
                        arm_action=arm_action_input,
                        arm_action_mask=arm_action_mask_input,
                        full_ref=full_ref,
                    )
